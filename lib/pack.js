...
function decodePack(repo, onEnd, read) {
  if (read === undefined)
    return decodePack.bind(this, repo, onEnd)
  ...
  function getObjectFromRefDelta(length, cb) {
    readChecksum(null, function (end, sourceHash) {
      ...
      readVarInt(readInflatedByte, function (err, expectedSourceLength) {
        ...
        readVarInt(readInflatedByte, function (err, expectedTargetLength) {
          if (err) return cb(err)
          // console.error('get obj')
          repo.getObject(sourceHash, function (err, sourceObject) {
            if (err) return cb(err)
            if (sourceObject.length != expectedSourceLength)
              return cb(new Error('Incorrect source object size in ref delta'))
            patchObject(b, length, sourceObject, expectedTargetLength, cb)
          })
        })
      })
    })
...
function patchObject(delta, deltaLength, srcObject, targetLength, cb) {
  var readByte = delta.chunks(1)
  var out = new Buffer(targetLength)
  var srcBuf, srcOffset = 0, outOffset = 0
  ...
  function gotByte(end, dBuf) {
    ...
    if (cmd & 0x80)
      ...
      readOffsetSize(cmd, delta, function (err, offset, size) {
        if (err) return earlyEnd(err)
        // console.error('off', offset, 'siz', size, srcOffset, outOffset, targetLength, srcObject.length)
        srcOffset += offset
        srcBuf.copy(out, outOffset, srcOffset, srcOffset + size)
        srcOffset += size
        outOffset += size
        readByte(null, gotByte)
      })
      ...
      delta.chunks(cmd)(null, function (err, buf) {
        if (err) return earlyEnd(err)
        buf.copy(out, outOffset)
        readByte(null, gotByte)
      })
    else
...
function readOffsetSize(cmd, delta, readCb) {
  var readByte = delta.chunks(1)
  var offset = 0, size = 0
    ...
    addByte(4, 0, function (val) {
      ...
      addByte(5, 1, function (val) {
        ...
        addByte(6, 2, function (val) {
          size |= val
          console.error('delta offset', offset, 'size', size)
          readCb(null, offset, size)
        })
      })
    })
  }
